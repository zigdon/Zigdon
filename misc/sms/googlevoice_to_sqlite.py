...
def parse_time (timestring):
    ...
    timestringmatch = re.search(r'(\d\d+):(\d\d):(\d\d)', timestring)
    seconds = 0
...
def process_file(tree, filename = None):
    ...
    if len(textNodes) > 0: #is actually a text file
        # Fizz Ellis - Placed - 2011-03-23T17_43_42Z.html
...
class gvoiceconn(sqlite3.Connection):
    ...
    def exportcsv(self):
        ...
        for view in views:
            ...
            with open(os.path.join('output', view[0]), 'wb') as csvfile:
...
if __name__ == '__main__':
    ...
    if os.path.exists(sys.argv[1]):
      conversationlocation = sys.argv[1]
    else:
    ...
    if not os.path.exists('output'):
        os.mkdir('output')
    gvconn = gvoiceconn(os.path.join('output', 'gvoice.sqlite')) #connect to sql database
        ...
        for i in getobjs(conversationlocation): #load each file into db, depending on type
            ...
            if isinstance(record, TextConversation): #set of text messages
                ...
                if gvconn.import_Audio(record, False) == 1: #no contact for audio. Can happen if files have been renamed
                    unmatched_audio.append(record) #save second pass for later
        del listline
